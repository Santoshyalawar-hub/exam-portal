...
import { Clock, User, BookOpen, AlertCircle, ChevronLeft, ChevronRight, Video, VideoOff, CheckCircle } from "lucide-react";
...
const QuizInterface = () => {
  ...
  const loadSavedData = () => {
    try {
      ...
      if (savedAnswers) {
        ...
        console.log("✅ Restored answers from session storage");
      }
  ...
  const clearExamSession = () => {
    try {
      if (studentId && liveExamId) {
        ...
        console.log("✅ Cleared exam session storage");
      }
    } catch (err) {
  ...
  }, [answers, timeLeft, currentQuestionIndex, tabSwitchCount]);
  ...
  // --- Handle Logout from Success Screen ---
  const handleLogoutFromSuccess = () => {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (res.data && res.data.userId) {
          ...
          // ✅ NEW: Fetch user details to get the actual name
          try {
            ...
            if (userRes.data) {
              ...
              console.log("✅ User details loaded:", userRes.data.name);
            }
          } catch (userErr) {
    ...
    const fetchExamData = async () => {
      try {
        ...
        try {
          ...
          console.log("ℹ️ No existing assignment");
        }
        ...
        // Check if there's saved time, otherwise set default
        ...
        // Load saved answers and progress AFTER questions are loaded
        loadSavedData();
  ...
  // --- SUCCESS SCREEN (Updated with Logo) ---
          ...
          <div className="max-w-4xl mx-auto">
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
              ...
              <div className="flex items-center justify-between pt-6 border-t border-gray-300 mt-8">
                ...
                {/* <button
                  onClick={!isLastQuestionOverall ? handleNext : undefined}
                  disabled={isLastQuestionOverall}
                  className={`px-6 py-2 rounded-lg transition-colors ${isLastQuestionOverall
                    ? "bg-gray-200 text-gray-400 cursor-not-allowed"
