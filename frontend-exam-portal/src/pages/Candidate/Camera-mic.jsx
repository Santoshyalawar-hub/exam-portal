...
function ExamStart() {
  ...
  const { userId, examId } = location.state || { userId: 1, examId: 11 };
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = () => {
        ...
        console.log("Recording URL:", URL.createObjectURL(blob));
      };
  ...
  const uploadPhotosToDatabase = async (verificationPhoto, idPhoto, userId, examId) => {
    try {
      ...
      console.log("Starting upload with:", { userId, examId });
      ...
      console.log("Blobs created:", {
        verificationSize: verificationBlob.size,
        idSize: idBlob.size
      });
      ...
      console.log("✅ Proctoring saved successfully:", result);
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === "hidden") {
        setTabSwitchCount((prev) => {
          ...
          setTabSwitchWarning(`Warning: Tab switch detected (${newCount}/3)`);
          if (newCount >= 3) {
            alert("Exam session terminated due to multiple tab switches.");
            navigate(`/exam-terminated?userId=${userId}&examId=${examId}&reason=tab_switching`);
          }
          return newCount;
        });
      }
        ...
        <div>
          ...
          {uploadStatus && (
            <div className={`mb-6 p-4 rounded-xl flex items-start ${
              uploadStatus.includes("successfully") || uploadStatus.includes("Session ID")
    ...
    <div className="bg-white shadow-2xl rounded-3xl p-10 text-center max-w-md w-full border border-gray-100">
      ...
      {tabSwitchWarning && (
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start">
          ...
          <p className="text-amber-800 text-sm font-medium">{tabSwitchWarning}</p>
        </div>
      )}
