...
function CandidateExamPortal() {
  ...
  // Fetch exam data (fixed exam ID = 11)
  useEffect(() => {
    const fetchExam = async () => {
      try {
        const res = await axios.get("http://localhost:8080/api/admin/exams/11/questions");
        setExam(res.data);
        setLoading(false);
      } catch (err) {
  ...
  const handleStart = () => {
    navigate(`/camera?examId=${exam.id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-28 container mx-auto px-6 py-8 max-w-7xl">
        ...
        {/* Header Section (moved slightly lower) */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
              <button
                onClick={systemReady ? handleStart : undefined}
              ...
              >
                {systemReady ? "Start Examination" : "Preparing System..."}
              </button>
              {systemReady && (
...
function StatusItem({ label, status, color, loading }) {
  return (
    <div className="flex items-center justify-between">
      ...
      {loading ? (
        ...
        <span className={`flex items-center text-${color}-600 font-medium`}>
