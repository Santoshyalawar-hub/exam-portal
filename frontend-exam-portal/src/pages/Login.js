...
//     useEffect(() => {
//     const storedUser = localStorage.getItem("truerize_user");
//     if (storedUser) {
...
// // figure out role and redirect page from backend
// const userRole = data.roles?.includes("ADMIN") ? "admin" : "candidate";
// const redirectPage = data.redirectPage === "admin-dashboard" ? "/dashboard" : "/candidate";
...
// };
// localStorage.setItem("truerize_user", JSON.stringify(userData));
...
//             {showWelcomeMessage && <div className="toast-message">Welcome to Truerize Online Exam Portal !!!</div>}
...
//                         {isLoading && <p className="text-blue-500 mt-2">Logging in...</p>}
...
//                     <video src={helpVideo} autoPlay muted loop playsInline className="login-video" />
//                 </div>
//             </div>
//         </div>
//     );
...
function LoginPageInternal() {
    ...
    // Make login page static (no scrolling)
    useEffect(() => {
    ...
    // ✅ FIXED: Prevent back navigation and clear any existing session
    useEffect(() => {
    ...
    // ✅ FIXED: Only check session if explicitly navigating (not after logout)
    useEffect(() => {
        const checkSession = async () => {
            // Don't check session if we just logged out
